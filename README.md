...
- **Native Home** (`/dashboard/home`) - what needs you (questions, stalled conversations, reached revisit dates), tasks to pick up, source-linked new replies, tasks gone quiet for 30+ days with Finished/Not doing it/Later/Still active outcomes and Undo, and a date-grouped checklist. Task states are derived from task momentum, what Tim does (opening a task, editing it, writing in its conversations), conversations and automation; never from read markers or checklist items. **All tasks** (`/dashboard/tasks`) groups every active task by state or group with bulk tidy actions. Home does not create a parallel Work/Commitment/Result system, mark conversations read merely by showing an excerpt, or infer completion from an idle session. The normal task/chat/tool experience remains authoritative.
