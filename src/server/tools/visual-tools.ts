...
import { emitVisualPublished, publishVisualFromToolArgs, visualPublishedToolResult, VISUAL_CONTENT_TYPE_SCHEMA, VISUAL_KIND_SCHEMA } from "./visual-tool-publisher.js";
import {
  defineBridgeTool,
...
export function createVisualToolDefinitions(ctx: AppContext): BridgeToolDefinition[] {
  return [
  defineSessionBridgeTool("publish_visual", {
    description:
    ...
    parameters: {
      type: "object",
      properties: {
        kind: { ...VISUAL_KIND_SCHEMA, description: "Visual artifact kind: \"image\", \"mermaid\", \"vega-lite\", or \"html\"." },
        ...
        content: { ...VISUAL_CONTENT_TYPE_SCHEMA, description: "Base64-encoded image bytes (image kind), plain text Mermaid source (mermaid kind), Vega-Lite JSON spec string or object (vega-lite kind), or plain text HTML (html kind)." },
