...
describe("computer use tools", () => {
  ...
  it("labels screenshots by their actual bytes (JPEG and PNG)", async () => {
    for (const { label, bytes, mimeType } of [
      { label: "JPEG", bytes: Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46, 0x49, 0x46, 0x00, 0x01]), mimeType: "image/jpeg" },
      { label: "PNG", bytes: Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0x00, 0x00, 0x00, 0x0d]), mimeType: "image/png" },
    ]) {
      nativeRequireMock.mockReturnValue(createComputerModule({
        screenshot: vi.fn(() => bytes),
      }));
      ...
      expect(result, label).toMatchObject({ type: "image", mimeType, data: bytes.toString("base64") });
      vi.resetModules();
    }
  });
