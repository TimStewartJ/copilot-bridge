...
import { formatStaleSignedThinkingUserMessage } from "../session-runner.js";
...
describe("SessionManager run state", () => {
  ...
  it("still recovers mid-turn when the failed session's disconnect is slow, evicting it to background cleanup", async () => {
    const sessionId = "session-claude-slow-disconnect-recovery";
    const { manager, telemetryStore } = createManager({ telemetry: true });
    const first = makeSession();
    const resumed = makeSession();
    first.session.listTasks = vi.fn().mockResolvedValue({ tasks: [] });
    let settleDisconnect: (() => void) | undefined;
    first.session.disconnect = vi.fn(() => new Promise<void>((resolve) => { settleDisconnect = resolve; }));
    manager.backend = {
      resumeSession: vi.fn()
        .mockResolvedValueOnce(first.session)
        .mockResolvedValueOnce(resumed.session),
    ...
    // The disconnect never answers: past the refresh deadline the dead handle is evicted instead of
    // failing the turn, and recovery proceeds on a fresh resume.
    await vi.advanceTimersByTimeAsync(15_000);
    await flushMicrotasks();

    expect(manager.sessionObjects.get(sessionId)).not.toBe(first.session);
    expect(manager.backend.resumeSession).toHaveBeenCalledTimes(2);
    expect(resumed.session.send).toHaveBeenCalledWith({
      prompt: expect.stringMatching(/Continue from where you left off/),
    });
    expect(manager.getSessionRunState(sessionId)).toBe("busy");
    expect(latestSpanMetadata(telemetryStore, "session.cache.disconnect", sessionId))
      .toMatchObject({ refresh: true, outcome: "timed-out" });
    expect(manager.cleanupOwnership.get(first.session)).toMatchObject({ sessionId });
    ...
    expect(latestSpanMetadata(telemetryStore, "session.run.recovery", sessionId))
      .toMatchObject({ outcome: "stale_signed_thinking_resumed" });
    resumed.getHandler()?.({ type: "session.idle", data: {}, timestamp: new Date(Date.now() + 1).toISOString() });
    await flushMicrotasks();
  });

  it("explains the stale-signature failure when live background tasks block mid-turn recovery", async () => {
    const sessionId = "session-claude-blocked-recovery";
    const { manager, eventBusRegistry, telemetryStore } = createManager({ telemetry: true });
    const first = makeSession();
    first.session.listTasks = vi.fn().mockResolvedValue({
      tasks: [{ id: "bg-1", status: "running", mode: "background" }],
    });
    first.session.disconnect = vi.fn().mockResolvedValue(undefined);
    manager.backend = { resumeSession: vi.fn().mockResolvedValueOnce(first.session) };
    ...
    expect(first.session.disconnect).not.toHaveBeenCalled();
    expect(manager.backend.resumeSession).toHaveBeenCalledTimes(1);
    expect(manager.getSessionRunState(sessionId)).toBe("idle");
    const terminal = bus.getTerminalState() as { complete: boolean; terminalType?: string; errorMessage?: string };
    expect(terminal).toMatchObject({ complete: true, terminalType: "error" });
    expect(terminal.errorMessage).toBe(formatStaleSignedThinkingUserMessage("cached session could not be disposed"));
    expect(terminal.errorMessage).toMatch(/Switch this session to another model or start a new chat/);
    expect(terminal.errorMessage).not.toMatch(/CAPIError/);
    expect(latestSpanMetadata(telemetryStore, "session.run.recovery", sessionId))
      .toMatchObject({ outcome: "stale_signed_thinking_recovery_failed", error: "cached session could not be disposed" });
  });
