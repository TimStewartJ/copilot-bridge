...
import { request, createTestApp, createMockSessionManager, eventually } from "../test-support/api-routes.js";
import { getDeviceHibernateCommand, requestDeviceHibernate } from "../server/platform.js";
import { cancelHibernate, disarmHibernateOnIdle, getHibernateOnIdleStatus, HIBERNATE_IDLE_POLL_INTERVAL_MS } from "../server/device-hibernate.js";
import type { AppContext } from "../server/app-context.js";

vi.mock("../server/platform.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("../server/platform.js")>();
  return {
    ...actual,
