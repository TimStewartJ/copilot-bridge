...
function CountPill({ label, value, tone }: { label: string; value: number; tone: string }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border border-border bg-bg-surface px-2 py-0.5 text-[11px] text-text-secondary ${value === 0 ? "opacity-50" : ""}`}>
...
const ROUND_BUTTON = "inline-flex h-9 min-w-9 items-center justify-center gap-1.5 rounded-full border px-2.5 text-xs transition-colors";
const NEUTRAL_BUTTON = `${ROUND_BUTTON} border-border bg-bg-surface text-text-secondary hover:bg-bg-hover hover:text-text-primary`;
  ...
  return (
    <div className="flex shrink-0 items-center gap-1.5">
      <button
        ...
        className={controller.micMuted ? `${ROUND_BUTTON} border-error/40 bg-error/10 text-error` : NEUTRAL_BUTTON}
      >
      ...
      <button
        ...
        className={`${ROUND_BUTTON} border-error/30 bg-error/10 text-error hover:bg-error/20`}
      >
        <PhoneOff size={14} />
        {!compact && "End"}
      </button>
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto mt-1.5 flex w-full max-w-4xl flex-wrap items-center gap-1.5">
        ...
        {view.metrics && (
          <button
            ...
            className={`ml-auto rounded-md px-2 py-0.5 text-[11px] ${showMetrics ? "bg-bg-hover text-text-primary" : "text-text-faint hover:bg-bg-hover hover:text-text-secondary"}`}
          >
            Latency
          </button>
        )}
      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        {view.notice && (
          <div role={view.notice.level === "error" ? "alert" : "status"} className={`mt-1.5 text-[11px] ${view.notice.level === "error" ? "text-error" : view.notice.level === "warning" ? "text-warning" : "text-text-muted"}`}>
            {view.notice.text}
          </div>
        )}
...
export function HandsFreePill({ controller, onOpenHelm, onEnd, bottomOffset }: {
  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex max-w-full items-center gap-2 rounded-full border border-border bg-bg-secondary/95 py-1.5 pl-1.5 pr-2 shadow-lg backdrop-blur">
        <button
          type="button"
          onClick={onOpenHelm}
          title="Back to Helm"
          className="flex min-w-0 items-center gap-2 rounded-full pr-1 text-left hover:opacity-90"
        >
    ...
    <section aria-label="Set up hands-free voice" className="shrink-0 border-t border-border bg-bg-secondary px-3 py-3 sm:px-4">
      <div className="mx-auto w-full max-w-4xl">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {!install ? (
              ...
              <button
                ...
                className="mt-3 inline-flex items-center gap-2 rounded-full bg-accent px-4 py-2 text-xs font-medium text-white hover:bg-accent-hover disabled:opacity-60"
              >
          ...
          <button type="button" onClick={onClose} className="shrink-0 rounded-md px-2 py-1 text-xs text-text-muted hover:bg-bg-hover hover:text-text-primary">Not now</button>
        </div>
      </div>
    </section>
  );
