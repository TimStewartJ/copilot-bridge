...
const AGENT_PROSE = "chat-prose prose prose-invert prose-xs max-w-none text-xs leading-relaxed text-text-secondary prose-headings:mb-1 prose-headings:mt-2 prose-p:my-1 prose-ul:my-1 prose-li:my-0 prose-pre:rounded prose-pre:bg-bg-primary prose-pre:p-2 prose-pre:text-[11px] prose-code:text-[11px]";

function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <div className="mb-1 text-[10px] font-medium uppercase tracking-wider text-text-faint">{children}</div>
  );
}
  ...
  return (
    <div className="min-w-0 text-[13px]" data-tool-status={unfinished ? "unfinished" : status ?? "context"}>
      <button
        ...
        className={`-mx-1.5 flex w-[calc(100%+0.75rem)] min-w-0 items-center gap-2 rounded-md px-1.5 py-1 text-left transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent/50 ${
      ...
      {expanded && (
        <div className="mb-2 ml-[7px] mt-1 space-y-3 border-l pl-4" style={{ borderLeftColor: "var(--color-agent-border)" }}>
          {instructions.length > 0 && (
            <div className="chat-scroll max-h-72 space-y-2 overflow-auto">
          ...
          {progressText && (
            <div>
              ...
              <pre className="chat-scroll max-h-32 overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] text-text-muted">
                {progressText}
              </pre>
            </div>
          )}
          ...
          {result && (
            <div>
              ...
              <div className={`${AGENT_PROSE} chat-scroll max-h-64 overflow-auto`}>
                <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]}>
