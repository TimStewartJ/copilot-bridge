...
export default function FocusQuietSources({ snapshot, ...props }: FocusInteractionProps & { snapshot?: FocusSnapshot }) {
  ...
  const fieldClass = cx(DS.field.input, DS.field.inputSize.md, "mt-1 min-h-11 min-w-0");
  return <section data-dashboard-panel="quiet-sources" className={DS.layout.section}>
    <h3><button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.ghost, "font-semibold")} aria-expanded={expanded} onClick={() => setExpanded((value) => !value)}>Quiet sources</button></h3>
      ...
      <section className="space-y-2">
        ...
        <form className="space-y-3" onSubmit={(event) => {
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <button type="submit" disabled={invalidSearch} className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")}>Find quiet concerns</button>
            <button type="button" aria-expanded={showFilters} aria-controls={filterId} className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} onClick={() => setShowFilters((value) => !value)}>Quiet filters</button>
            {filtered && <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} onClick={() => { setFilter({}); setDraft({}); }}>Clear quiet filters</button>}
          </div>
        ...
        {query.error && <p role="alert" className="text-xs text-error">Quiet concern inventory unavailable or incomplete: {query.error.message}
          <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.ghost, "ml-2 underline")} onClick={() => void query.refetch()}>Retry quiet concerns</button>
        </p>}
        ...
        {concerns.map((concern) => <article key={concern.id} className="min-w-0 space-y-1 rounded-lg border border-border p-3 text-xs text-text-muted">
          ...
          <div className="flex flex-wrap gap-2">
            {props.onInspectHistory && <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} onClick={() => props.onInspectHistory!(concern.id)}>Inspect concern History</button>}
            {props.onInspectHistoryFilter && <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} onClick={() => props.onInspectHistoryFilter!({
        ...
        {query.hasNextPage && <button type="button" disabled={query.isFetchingNextPage} className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} onClick={() => void query.fetchNextPage()}>{query.isFetchingNextPage ? "Loading..." : "Load more quiet concerns"}</button>}
        {props.onInspectHistoryFilter && <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.ghost, "text-accent underline")} onClick={() => props.onInspectHistoryFilter!({ lifecycle: "handed_off" })}>Inspect all handed-off work in History</button>}
      </section>
      <section className="space-y-2">
        ...
        {digests.length > 6 && <button type="button" className={cx(DS.button.base, DS.button.size.sm, DS.button.variant.secondary, "text-xs")} aria-expanded={showAllDigests} onClick={() => setShowAllDigests((value) => !value)}>{showAllDigests ? "Show fewer quiet digests" : `Show all ${digests.length} quiet digests`}</button>}
      </section>
    </div>}
  </section>;
}
