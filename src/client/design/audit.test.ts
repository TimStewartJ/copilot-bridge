...
describe("design rules", () => {
  it("flags each retired pattern", () => {
    const cases: Array<[string, string]> = [
      ...
      ["state-dot", '<span className="h-1.5 w-1.5 rounded-full bg-success" />'],
      ["state-dot", 'const busy = "size-2 rounded-full bg-info animate-pulse";'],
      ["state-dot", '<span className="h-2 w-2 rounded-full bg-identity-rose" />'],
      ["raw-palette", '<div className="text-sm text-red-500">Failed</div>'],
      ["raw-palette", 'const tone = "bg-blue-500/15 text-blue-400";'],
  ...
  it("leaves what the system itself uses alone", () => {
    const allowed = [
      ...
      '<span className="size-2 rounded-[2px] bg-identity-rose" />',
      '<div className="bg-success-surface text-success" />',
